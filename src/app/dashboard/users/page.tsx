...
import { Invite, ROLE_LABELS, UserProfile, UserRole } from "@/types";
...
export default function AdminUsersPage() {
  const { user: currentUser, canManageUsers, profileLoading } = useAuth();
  ...
  const handleChangeRole = async (u: UserProfile, newRole: UserRole) => {
    ...
    if (!confirm(`ยืนยันเปลี่ยน ${u.email} เป็น ${ROLE_LABELS[newRole]}?`)) return;
    try {
              ...
              >
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="flex items-center gap-2 mt-1 flex-wrap">
                      <span
                        className={`inline-flex items-center gap-1 text-xs font-medium px-2 py-1 rounded-full ${ROLE_BADGE[u.role]}`}
                      >
                        {ROLE_LABELS[u.role]}
                      </span>
                      {u.disabled && (
                ...
                <div className="flex flex-wrap gap-2 pt-1 border-t border-gray-100">
                  <select
                    ...
                    className="px-3 py-1.5 min-h-[36px] text-xs font-medium text-gray-700 bg-gray-50 border border-gray-200 rounded-md disabled:opacity-40 focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              ...
              >
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="flex items-center gap-2 mt-1 flex-wrap">
                      <span
                        className={`inline-flex items-center gap-1 text-xs font-medium px-2 py-1 rounded-full ${ROLE_BADGE[inv.role]}`}
                      >
